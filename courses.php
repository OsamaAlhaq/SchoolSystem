...
<nav class="navbar"> <!-- Start NavBar -->
  ...
  <div class="navbar-body">
    <ul class="nav navbar-nav">
      ...
      <li class="active"><a href="courses.php"><strong>Courses</strong></a></li>
    </ul>
  </div>
</nav>    <!-- end NavBar -->
...
<div class="container dashboard"> <!-- Start Dashboard -->
    ...
    <div class="row text-center">
        ...
        <div class="row search">
            <form action="courses.php">
                <input class="d_search" type="text" name="query" placeholder="Search Courses">
        ...
        <div class="row">
            <form action='courses.php'>
                ...
                if(!empty($_GET['type']) && (!strcasecmp('delete', $_GET['type']))) {
                    $query = "DELETE FROM course where CID = '" . $_GET['CID'] . "'and year ='" . $_GET['Year'] . "'";
                    ...
                    $query = "Select * from course where CID = '" . $_GET['query'] . "' or year = '" . $_GET['query'] . "' or room = '" . $_GET['query'] . "' or average = '" . $_GET['query']  . "'";
                else if(!empty($_GET['type']) && (!strcasecmp('insert', $_GET['type']))) {
                    $query = "INSERT INTO course (CID, year, room, average) VALUES ('" . $_GET['ID'] . "','" . $_GET['Year']. "','" . $_GET['Room'] ."','" . $_GET['Average']."')";
                    $result = $conn->query($query);
                    if(!$result)
                        echo "<br>" . $conn->error;
                    $query = "Select * from course";
                } else if(!empty($_GET['type']) && (!strcasecmp('update', $_GET['type']))) {
                    $query = "UPDATE course SET year = '" . $_GET['Year'] .  "', room = '" . $_GET['Room'] . "', average = '" . $_GET['Average']."' WHERE course . CID = " . $_GET['ID'];
                    $result = $conn->query($query);
                    if(!$result)
                        echo "<br>" . $conn->error;
                    $query = "Select * from course";
